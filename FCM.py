...
def create_presentation():
    ...
    def add_slide(title, content_points):
        ...
        tf.clear()  # Clear existing format
        ...
        for i, point in enumerate(content_points):
            ...
            p.level = 0
            if i == 0:
                # First point doesn't need extra spacing before
                p.space_before = Pt(6)
            else:
    ...
    # --- Slide 1: Cover ---
    add_section_header("管理組織的生存環境：生態與體制理論", "Chapter 06: 組織如何在叢林與社會中求生？")
    ...
    # --- Slide 3: Ecology Section ---
    add_section_header("第一部分：組織生態理論", "Organizational Ecology\n叢林法則：資源與利基的戰爭")

    # --- Slide 4: Carrying Capacity ---
    ...
    # --- Slide 5: Medical Case ---
    [cite_start]add_slide("【實務個案】台灣醫療機構的消長 [cite: 20-30]", [
        "現象 (2004-2014)：",
    ...
    # --- Slide 6: Inertia ---
    [cite_start]add_slide("結構慣性：為什麼大象難跳舞？ [cite: 99-109]", [
    ...
    # --- Slide 7: Strategy ---
    [cite_start]add_slide("生存策略：瑞士刀 vs. 手術刀 [cite: 136-146]", [
        "廣泛經營者 (Generalist)：",
        "   - 像瑞士刀，功能多、市場大。",
        "   - 資源寬裕，分散風險，但面臨激烈競爭。",
        "專精經營者 (Specialist)：",
    ...
    # --- Slide 8: Airline Case ---
    [cite_start]add_slide("【實務個案】台灣航空業的資源分割 [cite: 251-262]", [
    ...
    # --- Slide 9: Red Queen ---
    [cite_start]add_slide("紅皇后效應 (Red Queen Effect) [cite: 167-174]", [
    ...
    # --- Slide 10: Institution Section ---
    add_slide("第二部分：體制理論", [
        "Institutional Theory",
        "社會規則：不只搶錢，還要搶「面子」與「認同」"
    ])

    # --- Slide 11: Isomorphism ---
    [cite_start]add_slide("體制的三大支柱與同形化 [cite: 185-215]", [
    ...
    # --- Slide 12: Legitimacy ---
    [cite_start]add_slide("正當性 (Legitimacy)：組織的社會通行證 [cite: 273-280]", [
        "組織遵循體制，是為了換取「正當性」。",
        "三種正當性衡量：",
    ...
    # --- Slide 13: FB Case ---
    [cite_start]add_slide("【實務與理論對話】Facebook 的體制困境 [cite: 328-342]", [
        "Facebook 面臨的挑戰：",
    ...
    # --- Slide 14: Responses ---
    [cite_start]add_slide("教授錦囊：面對體制壓力的四種策略 [cite: 299-312]", [
        "1. 遵從 (Acquiesce)：",
