...
st.sidebar.caption("STEP 1: 資料來源")
...
st.sidebar.markdown("---")
# 使用 Expander 收納編輯工具
with st.sidebar.expander("🔧 矩陣編輯與亂數", expanded=False):
...
with tab1:
    st.subheader("矩陣權重檢視")
    st.caption("說明：數值範圍 -1.0 (抑制) 至 1.0 (促進)。")
...
with tab3:
    ...
    st.info("💡 使用說明：請依序點擊按鈕。程式會使用「長篇擴充邏輯」生成內容，並自動堆疊在下方。")
    ...
    if st.session_state.last_results is None:
        ...
        if c1.button("1️⃣ 生成 4.1 結構 (長篇)"):
            ...
            t += "綜上所述，結構分析證實了本研究模型具備清晰的「驅動-傳導-產出」邏輯層次，為後續的動態模擬提供了堅實的結構基礎。\n\n"
        ...
        # 4.3 按鈕
        if c3.button("3️⃣ 生成 4.3 情境模擬 (長篇)"):
        ...
        # 5.1 結論
        if c5.button("5️⃣ 生成 5.1 研究結論 (長篇)"):
            t = "### 第五章 結論與建議\n\n"
        ...
        # 5.2 建議
        if c6.button("6️⃣ 生成 5.2 管理意涵 (長篇)"):
        ...
        # 5.3 貢獻
        if c7.button("7️⃣ 生成 5.3 學術貢獻 (長篇)"):
        ...
        st.caption("說明：請依序點擊上方按鈕 (1~7)。內容會自動堆疊在此，形成約 5000-7000 字的完整文章架構。")
