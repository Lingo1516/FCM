...
# 0. 頁面初始化 (防止報錯)
# ==========================================
st.set_page_config(page_title="FCM 論文全功能系統", layout="wide")

st.markdown("""
<style>
    /* 論文預覽區：模擬 Word A4 格式，黑字白底 */
    .paper-preview { 
...
# 用 Dictionary 儲存長篇論文的每一節，確保順序正確且不遺失
...
# 1. 資料來源
st.sidebar.caption("STEP 1: 選擇資料")
mode = st.sidebar.radio("模式", ["內建真實模型", "上傳 Excel/CSV"], label_visibility="collapsed")
...
# 2. 編輯工具 (用 Expander 收起來)
st.sidebar.markdown("---")
with st.sidebar.expander("🔧 矩陣編輯工具", expanded=False):
    # 新增準則 (使用 Form 防止跳掉)
...
with tab2:
    ...
    if st.button("🚀 開始運算", type="primary"):
        ...
        ax.axhline(y=0, color='gray', linestyle='--', alpha=0.5) # 0軸線
...
# --- Tab 3: 長篇論文生成核心 (字數暴增版) ---
with tab3:
    st.subheader("🎓 論文分段生成器 (目標：7000字)")
    st.info("💡 操作說明：請依序點擊下方按鈕。每個按鈕都會生成一段長篇學術分析，並自動「接」在下方預覽區。")
    ...
    if st.session_state.last_results is None:
        ...
        st.caption("說明：上方按過的按鈕內容會自動堆疊在此處。請依序點擊以形成完整論文。")
        ...
        if full_text:
            ...
            st.download_button("📥 下載完整論文文字檔 (TXT)", full_text, "Full_Thesis_Draft.txt")
            
            if st.button("🗑️ 清空所有內容 (重新開始)"):
