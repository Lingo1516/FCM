...
with tab2:
    ...
    if st.button("🚀 開始運算", type="primary"):
        ...
        if np.all(st.session_state.matrix == 0):
            ...
            # 新增換算框框
            st.subheader("📈 换算激活值")
            selected_concept = st.selectbox("选择想验证的概念", st.session_state.concepts)
            selected_value = st.slider("输入激活值", 0.0, 1.0, 0.5)

            if st.button("计算激活值"):
                # 根据公式计算激活值
                idx = st.session_state.concepts.index(selected_concept)
                influence = np.dot(selected_value, st.session_state.matrix[idx, :])
                new_activation = sigmoid(influence, 1.0)  # 使用 Sigmoid 函数
                st.write(f"计算出的激活值: {new_activation:.4f}")
                st.write(f"验证结果: {new_activation:.4f} (理论值)")
