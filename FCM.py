...
mode = st.sidebar.radio("資料模式", ["內建真實模型", "上傳 Excel"])

if mode == "上傳 Excel":
    uploaded = st.sidebar.file_uploader("上傳", type=['xlsx', 'csv'])
    if uploaded:
        try:
...
with tab2:
    st.info("💡 請拉動 **A2 高層基調** 至 1.0，按開始運算。")
    cols = st.columns(3)
    initial_vals = []
...
with tab3:
    ...
    st.info("💡 使用說明：請依序點擊下方按鈕。每個按鈕都會生成一段長篇學術分析，並自動累積在下方預覽區。")
    ...
    if st.session_state.last_results is None:
        ...
        # 4.1 結構分析 (長篇版)
        if c1.button("1️⃣ 生成 4.1 結構分析 (長篇)"):
            t = "### 第四章 研究結果與分析\n\n"
            t += "**4.1 FCM 矩陣結構特性分析 (Structural Analysis)**\n\n"
            t += "本節旨在透過圖論 (Graph Theory) 指標，針對專家共識建立之模糊認知圖矩陣進行靜態結構檢測。此步驟之目的在於驗證系統邏輯的完整性，並識別出系統中的核心變數。\n\n"
            t += "**4.1.1 矩陣密度與連通性分析**\n"
        ...
        if st.session_state.paper_content:
            ...
            st.download_button("📥 下載完整論文文字檔 (TXT)", st.session_state.paper_content, "Full_Thesis_Draft.txt")
